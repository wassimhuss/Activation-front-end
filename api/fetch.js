export class Proxy {
  ...
  async Get_Table_By_OWNER_ID(i_Params_Get_Table_By_OWNER_ID) {
      this.url = this.APIBaseUrl + '/Get_Table_By_OWNER_ID?Ticket=' + (this.TicketMode === 'url' ? this.Ticket : '');
  ...
  async Delete_Tables(i_Params_Delete_Tables) {
      this.url = this.APIBaseUrl + '/Delete_Tables?Ticket=' + (this.TicketMode === 'url' ? this.Ticket : '');
      const result = await this.apiPost(this.url, i_Params_Delete_Tables).then(async (resp) => {
          return resp;
      });
      return result.any;
  }
  async Get_Extension_By_OWNER_ID(i_Params_Get_Extension_By_OWNER_ID) {
      this.url = this.APIBaseUrl + '/Get_Extension_By_OWNER_ID?Ticket=' + (this.TicketMode === 'url' ? this.Ticket : '');
      const result = await this.apiPost(this.url, i_Params_Get_Extension_By_OWNER_ID).then(async (resp) => {
          return resp;
      });
      return result.My_Result;
  }
  async Edit_Extension(i_Extension) {
      this.url = this.APIBaseUrl + '/Edit_Extension?Ticket=' + (this.TicketMode === 'url' ? this.Ticket : '');
      const result = await this.apiPost(this.url, i_Extension).then(async (resp) => {
          return resp;
      });
